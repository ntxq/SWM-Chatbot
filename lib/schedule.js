...
class Schedule {
  ...
  getRemainTime() {
    return Math.max(
      0,
      Math.round((this.deadline.getTime() - new Date().getTime()) / 60000)
    );
  }
...
exports.PersonalSchedule = class PersonalSchedule extends Schedule {
  constructor(_deadline, _conversationId, _content, _alarmPeriod = Infinity) {
    ...
    this.nextAlarmTime.setTime(this.nextAlarmTime.getTime() + this.alarmPeriod);
    if (
      isNaN(this.nextAlarmTime.getTime()) ||
      this.nextAlarmTime > this.deadline
    )
      this.nextAlarmTime.setTime(this.deadline);
...
exports.GroupSchedule = class GroupSchedule extends Schedule {
  ...
  async inviteNewMember(userId, name) {
    await libKakaoWork.inviteGroupConversation({
      conversation_id: this.conversationId,
      user_ids: [userId],
    });

    const formattedMessage = libKakaoWork.formatMessage(inviteMessage, {
      name,
    });
