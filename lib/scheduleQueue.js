    ...
    dbConnection.query("pr_all_todo", [], (err, row) => {
      ...
      if (result === 1) {
        ...
        scheduleArray.map((data) => {
          const conversationId = libKakaoWork.openConversations({
            userId: data.usr_id,
          });
          ...
          if (data.public === 0) {
            ...
            schdule.ID = data.v_ism_tdo_id;
            this._queue.push(schdule);
          } else {
            if (!groupIDs.has(data.v_ism_tdo_id)) {
              ...
              if (schedule) schedule.memberConversationId.push(conversationId);
            }
          }
        });
      }
