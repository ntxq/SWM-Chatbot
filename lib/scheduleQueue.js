...
class Schedule {
  ...
  sendMessage() {}
  setNextPeriod() {
    const remainTime = Math.round((this.deadline - new Date()) / 60000);
    //주기적 알람
    if (remainTime > 0) {
...
class PersonalSchedule extends Schedule {
  constructor(_deadline, _conversationId, _content, _alarmPeriod = Infinity) {
    ...
    const now = new Date();
    const timeDiff = super.deadline.getTime() - now.getTime();
    super.periodNum = Math.ceil(timeDiff / super.alarmPeriod);
    super.nextAlarmTime = super.deadline - super.periodNum * super.alarmPeriod;
    if (super.nextAlarmTime < now) {
      super.nextAlarmTime += super.alarmPeriod;
    }
  }

  async sendMessage() {
    const remainTime = Math.round((super.deadline - new Date()) / 60000);
